...
# https://github.com/pytorch/pytorch/pull/938
#==============================================================================
class CG(Optimizer):
    ...
    Arguments:
        ...
        beta_update_fn (str): beta update function, currently available
            ['FR','PR','HS',DY']
    ...
    def __init__(self, params, lr=1, max_iter=20, max_eval=None,
                 ...
                 beta_update_fn="FR",line_search_fn=None, bounds=None):
        if max_eval is None:
            max_eval = max_iter * 5 // 4
        defaults = dict(lr=lr, max_iter=max_iter, max_eval=max_eval,
                        tolerance_grad=tolerance_grad, tolerance_change=tolerance_change,
                        beta_update_fn=beta_update_fn,
                        line_search_fn=line_search_fn, bounds=bounds)
        super(CG, self).__init__(params, defaults)

        if len(self.param_groups) != 1:
            raise ValueError("CG doesn't support per-parameter options "
                             "(parameter groups)")
    ...
    def step(self, closure):
        ...
        beta_update_fn = group['beta_update_fn']
...
#        loss = orig_loss.data[0]
        ...
        while n_iter < max_iter:
            ...
            ############################################################
            if state['n_iter'] == 1: #First direction
                y = flat_grad.neg() #negative gradient
                d = y.clone() #direction_step
            else:
                y_old = y.clone() #old negative gradient
                d_old = d.clone() #old direction step
                y = flat_grad.neg()
                if beta_update_fn == 'FR':#Fletcher-Reeves
                    beta = (y**2).sum()/(y_old**2).sum()
                if beta_update_fn == 'PR':#Polak–Ribiere
                    beta = (y*(y - y_old)).sum()/(y_old**2).sum()
                if beta_update_fn == 'HS':#Hestenes-Stiefel
                    beta = -(y*(y - y_old)).sum()/(d_old*(y - y_old)).sum()
                if beta_update_fn == 'DY':#Dai–Yuan
                    beta = -(y**2).sum()/(d_old*(y - y_old)).sum()
                #direction reset
                beta = torch.max(torch.tensor(0.0),beta)
                d = y + beta*d_old
